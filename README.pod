...
Това хранилище съдържа файл C<.XCompose> с настройки за I<съставно въвѣжданѣ>
...
F<.XCompose> в сѫщата папка, кѫдѣто е и този файл, и пѿърсете в нѣго.
...
Поставетѣ горнитѣ двѣ опрѣдѣленꙗ въ файла C<~/.profile>. Така винаги, когато
влизатѣ въ вашата рабѿнa срѣда, тези промѣнливи ще ꙋказват на приложенꙗта да
зарѣждат файла C<~/.XCompose> или ꙋказанѭ файл въ стойността на промѣнливата на
обкрѫженѥто C<$XCOMPOSEFILE>. 

Сиреч, можетѣ или просто да поставитѣ този C<.XCompose> в домашната си папка,
или въ C<~/.profile> да добавитѣ и слѣднѭ рѣд (примѣр):
...
=head2 Как да въвѣждѫ старитѣ бꙋкви?

Преди да можетѣ да въвѣждатѣ стари бꙋкви само с помощта на клавꙗтурата, е
необходимо да зададетѣ/ꙋкажетѣ/настроитѣ вашиѭ I<Съставен клавиш> (Compose key).
...
C</usr/share/X11/xkb/rules/xorg.lst> има пълен списък с възможнитѣ клавиши.
Ето го.
...
Така в XFCE можетѣ хем да зададетѣ I<Съставен ключ>, хем да зададетѣ клавиш
(ключ) за трето ниво.

=over

B<Самото въвѣждане става по слѣднѭ начин:>

Нaтискате избранѭ I<съставен ключ>. Пускате го. Не го държите постоянно
натиснат. Слѣд това въвѣждате първата, а след неꙗ и втората бꙋква ѿ
послѣдователността.
...
=head1 СЪСТАВИЛ ТОВА РѪКОВОДСТВО

Красимир Беров <berov@cpan.org>

=head1 СЪСТАВИЛ .XCompose
